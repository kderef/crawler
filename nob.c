...
int main(int argc, char** argv) {
    ...
    // options passed by args
    bool run = streq(arg, "run");
    bool cross_compile = streq(arg, "cross");
    ...
    if (!nob_file_exists(libraylib) || cross_compile) {
        nob_log(INFO, "building raylib...");
        
        nob_cmd_append(&cmd, "make", "-C", PATH_RAYLIB "src/");

        if (cross_compile) nob_cmd_append(&cmd,
    ...
    if (cross_compile) target = TARGET_WINDOWS;

    // build main program
    typedef const char* cfg;

    cfg cc = cross_compile?
